...
<p align="center">
	<img src="./img/1.png" alt="Sample"  width="400">
	<p align="center">
...
- inputs: A tensor of size [batch, height, width, channels]. 
  ...
  - 为什么输入bottleneck之前是56 \* 56 \* 64* ? --> （见笔记的第一张图 [各种深度的ResNet](#img1)）因为ResNet接受的图像大小为224 * 224。经过第一层卷积层（input_channel=3, kernel_size=7x7, kernel_num=64, padding=3, stride=2）后，得$floor((224-7+3)/2)+1=112$。再经过一层池化（kernel_size=3, stride=2, padding=1），得$floor((112+2*1-3)/2)+1=56$。意味着：bottleneck的输入为$ 56（height） * 56(weight) * 64(channel) $ 的feature map。
